...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if not User.objects.filter(username='emre.ozzeybek').exists():
            user = User.objects.create_user(username='emre.ozzeybek', password='Emre', email='emre.ozzeybek@ibg.edu.tr')
            admin_group = Group.objects.get(name='RareIndex Admin')
            user.groups.add(admin_group)
        if not User.objects.filter(username='baris.salman').exists():
            user = User.objects.create_user(username='baris.salman', password='Baris', email='baris.salman@ibg.edu.tr')
            admin_group = Group.objects.get(name='RareIndex Admin')
            user.groups.add(admin_group)
        if not User.objects.filter(username='ayca.yigit').exists():
            user = User.objects.create_user(username='ayca.yigit', password='Ayça', email='ayca.yigit@ibg.edu.tr')
        ...
        if not User.objects.filter(username='mert.pekerbas').exists():
            user = User.objects.create_user(username='mert.pekerbas', password='Mert', email='mert.pekerbas@ibg.edu.tr')
            sensitive_group = Group.objects.get(name='RareIndex User(Sensitive Data)')
            user.groups.add(sensitive_group)
        if not User.objects.filter(username='kutay.bulut').exists():
            user = User.objects.create_user(username='kutay.bulut', password='Kutay', email='kutay.bulut@ibg.edu.tr')
            user_group = Group.objects.get(name='RareIndex User')
            user.groups.add(user_group)
        if not User.objects.filter(username='gennext').exists():
            user = User.objects.create_user(username='gennext', password='gennext')
        ...
        if not User.objects.filter(username='ezgi.karaca').exists():
            user = User.objects.create_user(username='ezgi.karaca', password='Ezgi', email='ezgi.karaca@ibg.edu.tr')

...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        self.stdout.write(self.style.SUCCESS(f"- {sum(len(s) for s in statuses.values())} Statuses:"))
        for model_type, model_statuses in statuses.items():
            self.stdout.write(self.style.SUCCESS(f"  - {len(model_statuses)} {model_type.title()} statuses"))
